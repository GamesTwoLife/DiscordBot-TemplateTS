...
export default new Event(
	...
	async (client: Client, interaction: Interaction) => {
		...
		try {
			...
			const selectmenus = client.components.cache
				.get(interaction.customId)
				?.filter((component) => component.data.type === 'selectmenu')
			...
			for (const selectmenu of selectmenus) {
				...
				if (
					selectmenu.options &&
					selectmenu.options.devGuildOnly &&
				...
				if (
					selectmenu.options &&
					selectmenu.options.bot_permissions &&
					!guild.members.me!.permissions.has(
						selectmenu.options?.bot_permissions
					)
				) {
					const permsBot = selectmenu.options?.bot_permissions
						?.map((x) => x)
						.join(', ')
				...
				if (
					selectmenu.options &&
					selectmenu.options.user_permissions &&
					!member.permissions.has(
						selectmenu.options?.user_permissions
					)
				) {
					const permsUser = selectmenu.options?.user_permissions
						?.map((x) => x)
						.join(', ')
				...
				const cooldownAmount =
					(selectmenu.options?.cooldown ?? 5) * 1000
				...
				return await (
					selectmenu as Component<AnySelectMenuInteraction<'cached'>>
				).execute(
					client,
